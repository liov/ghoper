...
func NewArticlePoster(posterName string, article *model.Article, qr *utils.QrCode) *ArticlePoster {
...
func (a *ArticlePoster) CheckMergedImage(path string) bool {
	if utils.CheckNotExist(path+a.PosterName) == true {
		return false
	}
...
func (a *ArticlePoster) OpenMergedImage(path string) (*os.File, error) {
	f, err := utils.MustOpen(a.PosterName, path)
	if err != nil {
		return nil, err
	}
...
func (a *ArticlePosterBg) Generate() (string, string, error) {
	...
	if !a.CheckMergedImage(path) {
		...
		bgF, err := utils.MustOpen(a.Name, path)
		if err != nil {
			return "", "", err
		}
		defer bgF.Close()

		qrF, err := utils.MustOpen(fileName, path)
		if err != nil {
			return "", "", err
		}
		defer qrF.Close()
