...
func UserRedisToDB() error {
	...
	ids, err := redis.Int64s(conn.Do("ZRANGEBYSCORE", model.LoginUser+"ActiveTime", "-inf", pastTime))
	if err != nil {
		return err
	}
	conn.Do("ZREMRANGEBYSCORE", model.LoginUser+"ActiveTime", "-inf", pastTime)
