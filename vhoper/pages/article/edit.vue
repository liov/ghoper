...
export default {
  ...
  async asyncData({ $axios,route }) {
    let res;
    if (route.query.id){
      res = await $axios.$get(`/api/article/` + route.query.id)
    }
  ...
  mounted() {
    if(this.$route.query.id) {
      this.editorType=this.article.content_type===0?'markdown':'html'
      this.categories = []
      for (let v of this.article.categories){
        this.categories.push(v.id)
      }
      this.article.categories= []
      this.tags=[]
      for (let v of this.article.tags){
        this.tags.push(v.name)
      }
      this.article.tags=[]
    }
  ...
  */
  ...
  methods: {
    ...
    async save(value, render) {
      ...
      const res = this.$route.query.id?await this.$axios.$put(`/api/article/${this.$route.query.id}`, this.article):
        await this.$axios.$post(`/api/article`, this.article)
          // success
      if (res.code === 200) {this.$router.push({ path: '/article' })}
