...
func Crawl(c *fasthttp.RequestCtx) {
	...
	if user.Name != initialize.ServerSettings.CrawlerName {
		SendErrJSON(c,"您没有权限执行此操作, 请使用爬虫账号")
		return
	}
...
func CrawlAccount(c *fasthttp.RequestCtx) {
	...
	if err := initialize.DB.Where("name = ?", initialize.ServerSettings.CrawlerName).Find(&users).Error; err != nil {
		fmt.Println(err.Error())
		SendErrJSON(c,"error")
		return
	}
...
func CreateAccount(c *fasthttp.RequestCtx) {
	...
	if len(users) <= 0 {
		var user model.User
		user.Name = initialize.ServerSettings.CrawlerName
